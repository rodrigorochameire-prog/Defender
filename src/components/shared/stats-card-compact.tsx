  ...
  return (
    <div
      ...
      className={cn(
        ...
        "group relative p-5 sm:p-4 rounded-xl bg-card border border-border",
    ...
    >
      ...
      <div className="flex items-start justify-between gap-4 sm:gap-3">
        <div className="flex-1 min-w-0 space-y-1.5 sm:space-y-1">
          ...
          <p className="text-xs sm:text-[10px] font-medium text-muted-foreground truncate uppercase tracking-wide group-hover:text-emerald-600/70 dark:group-hover:text-emerald-400/70 transition-colors duration-300">
            {label}
          </p>
          {loading ? (
            ...
            <p className="text-2xl sm:text-xl font-bold sm:font-semibold text-foreground">
              {value}
            </p>
          )}
          {(change || changeLabel) && (
        ...
        <div className="w-11 h-11 sm:w-9 sm:h-9 rounded-xl sm:rounded-lg bg-muted flex items-center justify-center flex-shrink-0 border border-border group-hover:border-emerald-300/30 dark:group-hover:border-emerald-700/30 group-hover:bg-emerald-50 dark:group-hover:bg-emerald-900/20 transition-all duration-300">
          <Icon className="w-5 h-5 sm:w-4 sm:h-4 text-muted-foreground group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors duration-300" />
        </div>
      </div>
    </div>
  );
