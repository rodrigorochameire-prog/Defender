...
export function DriveBreadcrumbs() {
  ...
  return (
    <div className="flex items-center gap-1 h-10 px-4 border-b border-zinc-800/50 bg-zinc-950 shrink-0 overflow-x-auto">
      {/* ─── Back Button ─── */}
      {ctx.breadcrumbPath.length > 0 && (
        <Button
          variant="ghost"
          size="icon"
          className="h-6 w-6 mr-1 text-zinc-500 hover:text-zinc-300 shrink-0"
      ...
      {atribuicao && (
        <>
          ...
          {ctx.breadcrumbPath.length > 0 && (
            <ChevronRight className="h-3.5 w-3.5 text-zinc-600 shrink-0 mx-0.5" />
          )}
        </>
      )}
