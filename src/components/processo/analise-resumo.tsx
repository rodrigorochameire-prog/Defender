  ...
  if (!hasContent) {
    return (
      <div className="text-center py-16">
        <Target className="h-10 w-10 mx-auto text-neutral-200 dark:text-neutral-700 mb-3" />
        <p className="text-sm text-neutral-400">Nenhuma análise disponível</p>
    ...
    <div className="space-y-6">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 mt-3">
            {[
              ...
              <div key={kpi.label} className="rounded-lg bg-neutral-50 dark:bg-neutral-800/40 p-3 text-center">
                <p className="text-2xl font-bold tabular-nums text-neutral-800 dark:text-neutral-100">{kpi.value}</p>
      ...
      {radarLiberdade && (
        <div className={`rounded-lg p-4 border-l-4 ${
          ...
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-sm font-semibold text-neutral-700 dark:text-neutral-200">Radar de Liberdade</span>
          ...
          <p className="text-[13px] text-neutral-500 dark:text-neutral-400 leading-relaxed">{radarLiberdade.detalhes}</p>
        </div>
      )}
      ...
      {(crimePrincipal || resumo) && (
        <div>
          ...
          <div className="mt-2 space-y-2">
            {crimePrincipal && (
              <p className="text-sm font-semibold text-neutral-800 dark:text-neutral-100">{crimePrincipal}</p>
            )}
            {resumo && (
              <p className="text-[13px] text-neutral-500 dark:text-neutral-400 leading-relaxed">{resumo}</p>
            )}
          </div>
        </div>
      )}
      ...
      {estrategia && (
        <div>
          ...
          <div className="mt-2 rounded-lg border border-neutral-200/60 dark:border-neutral-700/40 bg-neutral-50/30 dark:bg-neutral-800/20 p-4">
            <p className="text-[13px] text-neutral-600 dark:text-neutral-300 leading-relaxed">{estrategia}</p>
          </div>
        </div>
      )}
        ...
        <div>
          ...
          <ul className="mt-2 space-y-1">
            {saneamento.pendencias.map((p: string, i: number) => (
              <li key={i} className="text-[13px] text-neutral-500 dark:text-neutral-400 flex items-start gap-2">
        ...
        <div>
          ...
          <div className="mt-2 space-y-1.5">
            {checklistTatico.map((item, i) => (
              <div key={i} className="flex items-start gap-2.5 text-[13px] text-neutral-600 dark:text-neutral-300">
                <div className="h-4 w-4 mt-0.5 shrink-0 rounded border border-neutral-300 dark:border-neutral-600" />
  ...
  return (
    <div>
      ...
      <ul className="space-y-1.5">
        {items.map((item, i) => (
          <li key={i} className="flex items-start gap-2 text-[13px] text-neutral-500 dark:text-neutral-400">
            <Icon className="h-3.5 w-3.5 text-neutral-300 dark:text-neutral-600 mt-0.5 shrink-0" />
            <span className="leading-relaxed">{item}</span>
          </li>
        ))}
      </ul>
