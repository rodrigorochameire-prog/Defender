...
const statusLabels: Record<string, { label: string; color: string }> = {
  ...
  received: { label: "Recebido", color: "bg-neutral-100 text-neutral-600 dark:bg-neutral-800 dark:text-neutral-400" },
...
function SectionHeader({
  ...
  return (
    <button
      onClick={onToggle}
      className="w-full flex items-center gap-2 py-2 px-3 rounded-lg bg-neutral-50 dark:bg-neutral-900/50 hover:bg-neutral-100 dark:hover:bg-neutral-800/50 transition-colors cursor-pointer"
    >
      {isOpen ? (
      ...
      <span className="text-xs font-semibold text-neutral-700 dark:text-neutral-300 flex-1 text-left">
        {title}
      </span>
      <Badge className="bg-neutral-200 dark:bg-neutral-700 text-neutral-600 dark:text-neutral-300 text-[10px] px-1.5 py-0">
        {count}
      </Badge>
    </button>
  );
...
function DriveFileCard({ file }: { file: any }) {
  ...
  return (
    <div className="flex items-start gap-3 p-2.5 rounded-lg border border-neutral-200/80 dark:border-neutral-800/80 hover:border-neutral-300 dark:hover:border-neutral-700 transition-colors group">
      <div className="w-8 h-8 rounded-lg bg-neutral-100 dark:bg-neutral-800 flex items-center justify-center flex-shrink-0">
        <IconComp className="w-4 h-4 text-neutral-500 dark:text-neutral-400" />
      </div>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <p className="text-xs font-medium text-neutral-800 dark:text-neutral-200 truncate flex-1">
            {file.name}
          </p>
          {file.driveFileId && (
            <a
            ...
            >
              <ExternalLink className="w-3 h-3 text-neutral-400 hover:text-neutral-600 dark:hover:text-neutral-300" />
            </a>
          )}
        </div>
...
function RecordingCard({ recording }: { recording: any }) {
  ...
  return (
    <div className="rounded-lg border border-neutral-200/80 dark:border-neutral-800/80 hover:border-neutral-300 dark:hover:border-neutral-700 transition-colors overflow-hidden">
        ...
        <div className="flex-1 min-w-0">
          ...
          {recording.transcription && (
            <button
              ...
              className="text-[10px] text-neutral-500 hover:text-neutral-700 dark:hover:text-neutral-300 mt-1 flex items-center gap-1 cursor-pointer"
            >
              {showTranscript ? (
      ...
      {showTranscript && recording.transcription && (
        <div className="px-3 pb-2.5 pt-0">
          <div className="bg-neutral-50 dark:bg-neutral-900/50 rounded-md p-2 text-[10px] text-neutral-600 dark:text-neutral-400 max-h-40 overflow-y-auto font-mono leading-relaxed whitespace-pre-wrap">
            {recording.transcription}
          </div>
        </div>
      )}
...
function EmptyState({ icon: Icon, message }: { icon: React.ComponentType<{ className?: string }>; message: string }) {
  return (
    <div className="flex flex-col items-center justify-center py-6 text-center">
      <Icon className="w-8 h-8 text-neutral-300 dark:text-neutral-600 mb-2" />
...
export function TabMidia({ assistidoId, processoId, assistidoNome }: TabMidiaProps) {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-3">
      ...
      {/* ====================================== */}
      <div>
        <div className="w-full flex items-center gap-2 py-2 px-3 rounded-lg bg-neutral-50 dark:bg-neutral-900/50">
        ...
        <div className="mt-2 space-y-3">
          ...
          {transcriptionResult && (
            <div className="rounded-lg border border-neutral-200/80 dark:border-neutral-800/80 overflow-hidden">
              <div className="flex items-center justify-between px-3 py-1.5 border-b border-neutral-200/80 dark:border-neutral-800/80">
                ...
                <Button
                  variant="ghost"
                  size="sm"
                  className="h-6 px-2 text-[10px] text-neutral-500 hover:text-neutral-700 dark:text-neutral-400 dark:hover:text-neutral-200 cursor-pointer"
                  onClick={() => {
              ...
              <div className="bg-neutral-50 dark:bg-neutral-900/50 p-3 text-xs text-neutral-600 dark:text-neutral-400 max-h-48 overflow-y-auto leading-relaxed whitespace-pre-wrap">
                {transcriptionResult}
              </div>
            </div>
          )}
