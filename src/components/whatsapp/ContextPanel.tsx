...
export function ContextPanel({ contactId, configId, onClose }: ContextPanelProps) {
  ...
  return (
    <div className="w-[280px] flex-shrink-0 border-l border-zinc-800 bg-[#1c1c1f] flex flex-col h-full">
      {/* Header */}
      <div className="flex items-center justify-between px-3 py-2 border-b border-zinc-800">
        ...
        <button
          onClick={onClose}
          className="w-5 h-5 flex items-center justify-center rounded hover:bg-zinc-800 transition-colors text-zinc-500 hover:text-zinc-300 cursor-pointer flex-shrink-0"
        >
      ...
      {!isLoading && data?.assistido && (
        <div className="px-3 py-1.5 border-b border-zinc-800/50 bg-zinc-800/20">
      ...
      <div className="flex-1 overflow-hidden">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full gap-3 px-4 text-center">
            <div className="w-10 h-10 rounded-xl bg-zinc-800/60 border border-zinc-700 flex items-center justify-center">
              <LinkIcon className="h-5 w-5 text-zinc-500" />
            </div>
            <div>
              <p className="text-sm font-medium text-zinc-300">Contato não vinculado</p>
