...
export function SlashCommandMenu({
  ...
  if (filtered.length === 0) {
    return (
      <div
        ref={menuRef}
        className="rounded-lg border border-zinc-200 dark:border-zinc-700 shadow-lg bg-white dark:bg-zinc-900 overflow-hidden"
      >
  ...
  return (
    <div
      ...
      className="rounded-lg border border-zinc-200 dark:border-zinc-700 shadow-lg bg-white dark:bg-zinc-900 overflow-hidden"
    >
      <div className="px-2 py-1 text-[10px] font-semibold uppercase tracking-wider text-zinc-400 dark:text-zinc-500 border-b border-zinc-100 dark:border-zinc-800">
        Comandos
      </div>
      <ul className="py-1">
        {filtered.map((cmd, idx) => {
          ...
          return (
            ...
            >
              <Icon className={`h-4 w-4 shrink-0 ${isSelected ? "text-white" : "text-zinc-500 dark:text-zinc-400"}`} />
              <div className="flex-1 min-w-0">
                <span className={`text-sm font-medium ${isSelected ? "text-white" : ""}`}>
                  /{cmd.name}
                </span>
                <span className={`ml-2 text-xs ${isSelected ? "text-zinc-200" : "text-zinc-500 dark:text-zinc-400"}`}>
                  {cmd.description}
                </span>
              </div>
            </li>
