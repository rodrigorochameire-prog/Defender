 ...
 *    às 09h50min."
 *
 * Variações cobertas: "designo/redesigno/fica designada/aprazo", tipos de
 * audiência usuais, datas "dia DD/MM/AAAA", horas "09h50min" / "9h" / "09:50",
 * modalidade (híbrida/virtual/presencial/videoconferência).
 *
...
const GATILHO =
  /\b(?:(re)?designo|(re)?designa(?:da|-se)?|fica(?:m)?\s+(re)?designad[ao]s?|aprazo|aprazada)\b[\s\S]{0,80}?\baudi[eê]ncia\b|\baudi[eê]ncia\b[\s\S]{0,80}?\b(re)?designad[ao]\b/i;
...
export function detectarDesignacaoAudiencia(
  ...
  const dm = DATA_RE.exec(aPartir) ?? DATA_FALLBACK_RE.exec(aPartir);
  if (!dm) return null;
  ...
  // Hora: procurada após a data (padrão "dia X, às HHhMM").
  const aposData = aPartir.slice((dm.index ?? 0) + dm[0].length);
  const hm = HORA_RE.exec(aposData);
