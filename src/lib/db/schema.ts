...
export const calendarEvents = pgTable("calendar_events", {
  ...
  assistidoId: integer("assistido_id").references(() => assistidos.id, { onDelete: "cascade" }),
  demandaId: integer("demanda_id").references(() => demandas.id, { onDelete: "set null" }),
  // Relacionamentos pet care (compatibilidade)
...
// ==========================================
// PETS & TUTORES (TeteCare)
// ==========================================

export const pets = pgTable("pets", {
  id: serial("id").primaryKey(),
  name: varchar("name", { length: 100 }).notNull(),
  species: varchar("species", { length: 50 }).default("dog").notNull(),
  breed: varchar("breed", { length: 100 }),
  birthDate: timestamp("birth_date"),
  weight: integer("weight"), // em gramas
  photoUrl: text("photo_url"),
  status: varchar("status", { length: 50 }).default("active").notNull(), // 'active' | 'checked-in'
  approvalStatus: varchar("approval_status", { length: 50 }).default("pending").notNull(),
  credits: integer("credits").default(0).notNull(),
  foodBrand: varchar("food_brand", { length: 200 }),
  foodAmount: integer("food_amount"),
  notes: text("notes"),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
}, (table) => [
...
export const creditPackages = pgTable("credit_packages", {
  ...
  index("credit_packages_display_idx").on(table.displayOrder),
]);
...
export const bookingRequests = pgTable("booking_requests", {
  ...
  requestType: varchar("request_type", { length: 50 }).default("daycare").notNull(),
...
export const dailyLogs = pgTable("daily_logs", {
  ...
  source: varchar("source", { length: 20 }).notNull(), // 'home' | 'daycare'
  mood: varchar("mood", { length: 50 }),
  stool: varchar("stool", { length: 50 }),
...
export const vaccineLibrary = pgTable("vaccine_library", {
  ...
  dosesRequired: integer("doses_required").default(1).notNull(),
  createdAt: timestamp("created_at").defaultNow().notNull(),
}, (table) => [
  index("vaccine_library_name_idx").on(table.name),
]);
...
export const petVaccinations = pgTable("pet_vaccinations", {
  ...
  vaccineId: integer("vaccine_id")
    .notNull()
    .references(() => vaccineLibrary.id, { onDelete: "cascade" }),
  ...
  index("pet_vaccinations_next_due_idx").on(table.nextDueDate),
]);
...
export const trainingLogs = pgTable("training_logs", {
  ...
  index("training_logs_pet_idx").on(table.petId),
  index("training_logs_date_idx").on(table.logDate),
...
export const trainingCommands = pgTable("training_commands", {
  ...
  uniqueIndex("training_commands_name_idx").on(table.name),
  index("training_commands_category_idx").on(table.category),
]);
...
export const petFoodPlans = pgTable("pet_food_plans", {
  ...
  petId: integer("pet_id")
    .notNull()
    .references(() => pets.id, { onDelete: "cascade" }),
  foodType: varchar("food_type", { length: 50 }).notNull(),
  brand: varchar("brand", { length: 200 }).notNull(),
  productName: varchar("product_name", { length: 200 }),
  dailyAmount: integer("daily_amount").notNull(),
  portionsPerDay: integer("portions_per_day").default(2).notNull(),
  portionTimes: text("portion_times"),
  notes: text("notes"),
  isActive: boolean("is_active").default(true).notNull(),
  ...
  index("pet_food_plans_pet_id_idx").on(table.petId),
  index("pet_food_plans_active_idx").on(table.petId, table.isActive),
]);
...
export const petFoodInventory = pgTable("pet_food_inventory", {
  ...
  index("pet_food_inventory_pet_id_idx").on(table.petId),
  index("pet_food_inventory_received_idx").on(table.petId, table.receivedDate),
]);
...
export const petFoodHistory = pgTable("pet_food_history", {
  ...
  digestion: varchar("digestion", { length: 20 }),
  stoolQuality: varchar("stool_quality", { length: 20 }),
  coatCondition: varchar("coat_condition", { length: 20 }),
  ...
  index("pet_food_history_pet_id_idx").on(table.petId),
  index("pet_food_history_created_idx").on(table.petId, table.createdAt),
]);
...
export const petTreats = pgTable("pet_treats", {
  ...
  petId: integer("pet_id")
    .notNull()
    .references(() => pets.id, { onDelete: "cascade" }),
  treatType: varchar("treat_type", { length: 50 }).notNull(),
  name: varchar("name", { length: 200 }).notNull(),
  brand: varchar("brand", { length: 200 }),
  purpose: varchar("purpose", { length: 100 }),
  frequency: varchar("frequency", { length: 50 }),
  maxPerDay: integer("max_per_day"),
  caloriesPerUnit: integer("calories_per_unit"),
  acceptance: varchar("acceptance", { length: 20 }),
  notes: text("notes"),
  isActive: boolean("is_active").default(true).notNull(),
  createdById: integer("created_by_id")
  ...
  index("pet_treats_pet_id_idx").on(table.petId),
  index("pet_treats_active_idx").on(table.petId, table.isActive),
]);
...
export const petNaturalFood = pgTable("pet_natural_food", {
  ...
  proteinSource: varchar("protein_source", { length: 200 }),
  ...
  index("pet_natural_food_pet_id_idx").on(table.petId),
  index("pet_natural_food_active_idx").on(table.petId, table.isActive),
]);
