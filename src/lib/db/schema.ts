...
// Enums para diligências
export const categoriaDiligenciaEnum = pgEnum("categoria_diligencia", [
  "SOCIAL",       // Pesquisa em redes sociais e fontes abertas
  "CAMPO",        // Visitas presenciais e coleta de informações in loco
  "OFICIAL",      // Solicitações oficiais a órgãos públicos
  "GEO",          // Análise de dados de localização e ERBs
  "TELEFONIA",    // Quebra de sigilo telefônico e análise de registros
  "DOCUMENTAL",   // Análise de documentos dos autos
  "PERICIAL",     // Perícias e laudos técnicos
  "TESTEMUNHAL",  // Localização e preparação de testemunhas
]);

export const statusDiligenciaEnum = pgEnum("status_diligencia", [
  "NAO_INICIADA",
  "EM_ANDAMENTO",
  "AGUARDANDO",
  "CONCLUIDA",
...
export const executorDiligenciaEnum = pgEnum("executor_diligencia", [
  "DEFENSOR",
  "SERVIDOR",
  "ESTAGIARIO",
  "FAMILIA",
...
export const diligencias = pgTable("diligencias", {
  ...
  // Classificação
  categoria: categoriaDiligenciaEnum("categoria").notNull(),
  status: statusDiligenciaEnum("status").default("NAO_INICIADA").notNull(),

  // Executor
  executor: executorDiligenciaEnum("executor").default("DEFENSOR"),
  executorNome: text("executor_nome"),
  executorContato: text("executor_contato"),
  ...
  checklist: jsonb("checklist").$type<{ id: string; texto: string; concluido: boolean }[]>(),

  // Resultado
  notas: text("notas"),
  resultado: text("resultado"),

  // Arquivos anexados (JSON array)
  arquivos: jsonb("arquivos").$type<{ id: string; nome: string; url: string }[]>(),
  ...
  index("diligencias_processo_id_idx").on(table.processoId),
  index("diligencias_workspace_id_idx").on(table.workspaceId),
  index("diligencias_categoria_idx").on(table.categoria),
  index("diligencias_status_idx").on(table.status),
  index("diligencias_prazo_idx").on(table.prazo),
  index("diligencias_responsavel_id_idx").on(table.responsavelId),
  index("diligencias_deleted_at_idx").on(table.deletedAt),
]);
...
export const diligenciasRelations = relations(diligencias, ({ one }) => ({
  ...
  responsavel: one(users, { fields: [diligencias.responsavelId], references: [users.id] }),
  createdBy: one(users, { fields: [diligencias.createdById], references: [users.id] }),
}));
