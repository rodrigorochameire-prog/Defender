      ...
      <div className="sticky top-0 z-20 px-4 md:px-6 py-3 bg-white/95 dark:bg-zinc-900/95 backdrop-blur-sm border-b border-zinc-200/80 dark:border-zinc-800/80">
        <div className="flex items-center justify-between max-w-6xl mx-auto">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <span className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">Inteligência do Júri</span>
              <span className="text-xs text-zinc-400 ml-2">Padrões cruzados</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto p-4 md:p-6 space-y-6">
            ...
            {argumentosTop.mp.length > 0 ? (
              <div className="space-y-3">
                {argumentosTop.mp.map((arg, i) => (
                  <div key={arg.id} className="flex items-start gap-3 pb-2 border-b border-zinc-100 dark:border-zinc-800 last:border-0">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {juradosMaisPrevistos.map((j) => (
                <Link key={j.id} href={`/admin/juri/jurados/${j.id}`} className="block">
                  <div className="rounded-lg border border-zinc-100 dark:border-zinc-800 p-3 hover:border-emerald-200 dark:hover:border-emerald-800 transition-colors cursor-pointer">
                    <div className="flex items-center justify-between mb-2">
        ...
        <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-5">
          ...
          {timeline.length > 0 ? (
            <div className="space-y-2">
              {timeline.map((s) => (
                <div key={s.id} className="flex items-center gap-3 py-2 border-b border-zinc-100 dark:border-zinc-800 last:border-0">
                  <div className={cn(
