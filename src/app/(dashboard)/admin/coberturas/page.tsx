...
function getStatus(dataInicio: string, dataFim: string | null, ativo: boolean): { label: string; color: string } {
  if (!ativo) return { label: "Encerrada", color: "bg-neutral-100 dark:bg-muted text-neutral-500" };
  const hoje = new Date();
  const inicio = parseISO(dataInicio);
  if (isAfter(inicio, hoje)) return { label: "Futura", color: "bg-sky-100 dark:bg-sky-900/30 text-sky-700 dark:text-sky-400" };
  if (dataFim && isBefore(parseISO(dataFim), hoje)) return { label: "Encerrada", color: "bg-neutral-100 dark:bg-muted text-neutral-500" };
    ...
    <Card className="bg-white dark:bg-card border-neutral-200/80 dark:border-border/80 rounded-xl overflow-hidden hover:border-emerald-200/50 dark:hover:border-emerald-800/30 transition-all duration-200">
      <div className="p-4">
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-[10px] text-neutral-400">{cobertura.tipo || "Ferias"}{cobertura.motivo ? ` — ${cobertura.motivo}` : ""}</span>
        </div>
        ...
        <div className="flex items-center gap-3 mb-3">
          <div className="flex items-center gap-2 flex-1">
            <Avatar className="h-8 w-8">
              <AvatarFallback className="text-[10px] bg-neutral-100 dark:bg-muted text-neutral-600 dark:text-foreground/80 font-medium">
                {getInitials(defensorNome)}
              </AvatarFallback>
            </Avatar>
            <div>
              <p className="text-[10px] text-neutral-400 uppercase tracking-wide">Afastado</p>
              <p className="text-xs font-medium text-neutral-700 dark:text-foreground/80">{defensorNome}</p>
            </div>
          </div>

          <ArrowRight className="w-4 h-4 text-neutral-300 dark:text-muted-foreground/50 flex-shrink-0" />
          ...
          <div className="flex items-center gap-2 flex-1">
            ...
            <div>
              <p className="text-[10px] text-neutral-400 uppercase tracking-wide">Cobrindo</p>
              <p className="text-xs font-medium text-neutral-700 dark:text-foreground/80">{substitutoNome}</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between pt-2 border-t border-neutral-100 dark:border-border">
          ...
          {cobertura.ativo && st.label === "Ativa" && (
            <Button
              variant="ghost"
              size="sm"
              className="h-6 text-[10px] text-neutral-400 hover:text-rose-500 cursor-pointer"
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 rounded-xl bg-neutral-900 dark:bg-white flex items-center justify-center shadow-lg">
              <ArrowLeftRight className="w-5 h-5 text-white dark:text-neutral-900" />
            </div>
            <div>
              <h1 className="text-xl font-bold text-neutral-900 dark:text-foreground tracking-tight font-serif">Coberturas</h1>
              <p className="text-xs text-neutral-500 dark:text-muted-foreground">Afastamentos e substituicoes</p>
            </div>
          </div>

          <Button
            size="sm"
            className="h-8 px-3 bg-neutral-900 hover:bg-emerald-600 dark:bg-neutral-100 dark:text-neutral-900 dark:hover:bg-emerald-500 text-white text-xs cursor-pointer"
      ...
      <div className="p-4 md:p-6 max-w-3xl mx-auto space-y-6">
        {isLoading ? (
          ...
          <>
            ...
            {ativas.length > 0 && (
              <div>
                <p className="text-[10px] font-medium text-neutral-400 uppercase tracking-wider mb-2">Ativas agora</p>
            ...
            {futuras.length > 0 && (
              <div>
                <p className="text-[10px] font-medium text-neutral-400 uppercase tracking-wider mb-2">Programadas</p>
                <div className="space-y-3">
                  {futuras.map(c => (
                    <CoberturaCard key={c.id} cobertura={c} onEncerrar={(id) => encerrarMutation.mutate({ id })} />
                  ))}
            ...
            {encerradas.length > 0 && (
              <div>
                <p className="text-[10px] font-medium text-neutral-400 uppercase tracking-wider mb-2">Encerradas</p>
                <div className="space-y-3">
            ...
            {(coberturas ?? []).length === 0 && (
              <Card className="bg-white dark:bg-card border-neutral-200/80 dark:border-border/80 rounded-xl p-8 text-center">
                <ArrowLeftRight className="w-12 h-12 mx-auto text-neutral-300 dark:text-muted-foreground/50 mb-3" />
                <p className="text-sm font-medium text-neutral-500">Nenhuma cobertura registrada</p>
                <p className="text-xs text-neutral-400 mt-1">Registre afastamentos para gerenciar coberturas</p>
                <Button
                  size="sm"
                  className="mt-4 h-8 text-xs bg-neutral-900 hover:bg-emerald-600 dark:bg-neutral-100 dark:text-neutral-900 dark:hover:bg-emerald-500 text-white cursor-pointer"
                  onClick={() => setCoberturaModalOpen(true)}
                >
