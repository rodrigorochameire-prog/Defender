...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    // 3. Resolve defensor
    // Aceita admin OU defensor (Rodrigo é admin mas atua como defensor).
    ...
    // Não há mais "primeiro defensor da tabela" — esse comportamento já causou
    // bug onde 84+ demandas foram atribuídas para a defensora errada (Cristiane id=2)
    // por estar fisicamente primeiro no heap quando o webhook não trazia email válido.
    if (!defensorId) {
