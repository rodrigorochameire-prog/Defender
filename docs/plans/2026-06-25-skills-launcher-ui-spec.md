...
- 2026-06-25: **Stories 6–9 concluídas (continuação "SIGA").** Histórico de tasks por entidade: `analise.recentForEntity` (query + 5 testes de schema CI-safe), `task-history.ts` (formatter puro, 13 testes), `SkillTaskHistory` (4 testes RTL). Superfícies: histórico no `ProcessoSheet` (abaixo do launcher) + launcher de assistido (skills `assistido`/`ANY`) + histórico no cockpit de análise do assistido. Gate verde: `tsc` 0 · `CI=1 vitest` 263 arq/**1993** testes (0 falhas) · `next lint` 0.
- Próximos (fora desta sessão): command palette ⌘K (plano 2026-03-27), `leitura-para-peca` (botão "gerar minuta" a partir do caderno de citações), migração de references (`skills-defensoria-refactor`).
