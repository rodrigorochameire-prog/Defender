...
- **Workspace no dedup:** a merge-queue existente (`suggestMerges`, `pessoas.ts:397`) é workspace-agnóstica (self-join global por nome). **Decisão de implementação do piloto (divergência consciente do desenho inicial):** o pool `existentes` da promoção automática é **escopado por workspace** — é a direção mais conservadora no caminho de *escrita* (nunca cross-linka pessoas entre workspaces; um link errado grava participação, um link perdido apenas vai para a merge-queue, que reconcilia globalmente). Novas pessoas herdam o `workspaceId` do processo. OMBUDS é majoritariamente single-workspace, então o efeito prático é nulo e o risco fica menor.
