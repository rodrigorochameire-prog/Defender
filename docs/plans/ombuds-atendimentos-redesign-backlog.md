...
- [x] **5.0 Lógica pura extraída** (`gerar-demanda-logic.ts`, 9 testes): `montarRegistroDoAtendimento` (movido do popover, HTML limpo), `atribuicaoAtosLabel`, e `filtrarAtos` (busca acento/caixa-insensível + agrupamento por categoria) — fundação testada do ProceduralActSelector. Popover já consome as duas primeiras. [x] `buildCreateFromFormPayload` extraído (firewall do payload de `createFromForm`) — réplica fiel da regra inline, `CreateFromFormPayload` com campos opcionais espelhando o schema do tRPC; 7 testes por branch (processoId/numeroAutos/status/prazo/providencias/atendimentoId/ato). Consumido na CTA do Sheet.
- [x] 5.1 **Popover → Sheet** (`gerar-demanda-popover.tsx`): superfície nobre lateral (`w-full sm:max-w-md`, full-height) em 5 blocos rotulados — **Origem** (header fixo: título + assistido) · **Definição jurídica** (atribuição + ato) · **Regime de urgência** (prazo + flags) · **Conteúdo inicial** (registro + vínculo) · **Confirmação** (recap: ato/atribuição/prazo/flags/vínculo) — corpo rolável + **CTA ancorado no `SheetFooter`**. API pública intacta (`children` como `SheetTrigger asChild` → caller em `atendimento-detail-sheet.tsx` sem mudança). Payload `createFromForm` + onSuccess + auto-vínculo **inalterados**. _Pendente_: extrair `buildCreateFromFormPayload` (firewall) + verificar foco em sheet-dentro-de-sheet no device.
