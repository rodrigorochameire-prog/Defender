...
- **Polaridade (deferido × indeferido):** o parser implementa uma **máquina de polaridade por cláusula** (estado inicial "defere"). A segmentação inclui split em `;` além de alíneas, incisos romanos e fronteiras de frase (`.`). Para cada cláusula em ordem: negação tem precedência (ambos os sinais → "indefere"); verbo explícito de deferimento (`defiro`, `determino`, `concedo`…) sobrepõe polaridade herdada; cláusula sem verbo herda a polaridade anterior (cobre listas governadas por verbo preamble, ex.: "DEFIRO: x; y"). Medidas **indeferidas nunca são gravadas como deferidas** (sem falso-positivo — o dano crítico evitado). O falso-negativo por deny+grant separados por `;` **foi corrigido** (ex.: "Indeferida a medida X; defiro o afastamento" → captura corretamente o afastamento). Caso residual intencional: deny+grant na mesma cláusula única sem `;` (ex.: "Indefiro o contato e defiro o afastamento" numa só cláusula) ainda é tratado como "indefere" por toda a cláusula — falso-negativo seguro, pois o defensor confirma no preview antes de gravar.
