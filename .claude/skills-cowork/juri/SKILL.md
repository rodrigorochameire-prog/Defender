...
## Identidade Visual da Atribuição Tribunal do Júri
...
### Paleta Tribunal do Júri

| Elemento | Cor | Hex | Uso |
|---|---|---|---|
| **Cor-tema** | Verde escuro | `1A5C36` | Banner principal, borda esquerda de headings, texto de títulos |
| **Fundo suave** | Verde menta claro | `EAF5EE` | Fundo de tabelas de identificação, headings, rodapé |
| **Cor clara** | Verde claro | `A8D5B5` | Subtítulo no banner, borda top de subheadings |
| **OMBUDS UI** | Green-600 | `16A34A` | Badge de atribuição no app web (de `atribuicoes.ts`) |

### Paleta Grupo Especial do Júri

| Elemento | Cor | Hex | Uso |
|---|---|---|---|
| **Cor-tema** | Laranja | `EA580C` | Banner, headings, títulos |
| **Fundo suave** | Laranja claro | `FFF7ED` | Fundos |
| **Cor clara** | Laranja suave | `FDBA74` | Subtítulos, bordas |
| **OMBUDS UI** | Orange-600 | `EA580C` | Badge no app web |

### Aplicação obrigatória em .docx (relatórios e análises)

1. **Banner**: fundo `1A5C36`, título branco, subtítulo `A8D5B5`
2. **Tabela de identificação**: fundo `EAF5EE`, rótulos cinza, valores bold `1A5C36`
3. **Headings de seção**: fundo `EAF5EE`, borda esquerda 32pt `1A5C36`, texto `1A5C36`
4. **Subheadings**: texto bold `1A5C36`, borda top `A8D5B5`
5. **Termos em destaque** (bold inline): cor `1A5C36`
6. **Rodapé do relatório**: fundo `EAF5EE`, borda top `1A5C36`

> Para Grupo Especial do Júri, substituir verde por laranja (`EA580C` / `FFF7ED` / `FDBA74`).

### Aplicação em .md (relatórios markdown)

- Título: `# TRIBUNAL DO JÚRI — Análise Estratégica — [NOME DO DEFENDIDO]`
- Subtítulo: `**Dossiê Estratégico de Defesa — [Fase: Sumário da Culpa / Plenário / etc.]**`
...
4. **Localizar o processo do assistido** (se disponível) — Busque a pasta do assistido nos diretórios de processos (ver "Pastas de Processos") e leia PDFs, transcrições (.md, .mp4) e documentos para extrair fatos, nomes, datas, depoimentos e argumentos do caso
5. **Consultar modelos reais similares** — Busque peças/análises do mesmo tipo em "Petições por assunto" (ver "Banco de Modelos") para absorver o padrão argumentativo e estilo do Defensor
6. **Coletar informações complementares** — Se os autos/documentos não forem suficientes, peça dados adicionais ao usuário
7. **Gerar a minuta/análise** — Siga o prompt carregado, baseando-se nos fatos do processo e no estilo dos modelos reais
8. **Formatar conforme o tipo** — Peças processuais em .docx (`_shared/formatacao-dpe-ba.md`), análises com cores do Júri (`_shared/padrao-relatorio.md`)
