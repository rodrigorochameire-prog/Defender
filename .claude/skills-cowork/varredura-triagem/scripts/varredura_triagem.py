        ...
        for i, d in enumerate(demandas):
            try:
                ...
                if atrib_demanda == "EXECUCAO_PENAL":
                    ...
                    texto = _clean_decisao_text(content["text"])
                    # PDF-fallback (A): se a leitura por frame veio vazia (doc só-PDF,
                    # ex. peças de Júri escaneadas), baixa o PDF de DENTRO dos autos
                    # completos (mesma URL já validada — não efetiva ciência) e extrai
                    # por pdftotext/OCR. baixar_pdf_autos reaplica a trava anti-ciência
                    # (só listProcessoCompletoAdvogado.seam) como segunda barreira.
