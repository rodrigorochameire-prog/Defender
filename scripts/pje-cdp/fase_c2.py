#!/usr/bin/env python3
"""
Fase C2 — retry robusto do download direto:
- confirma que o diálogo abriu (texto 'Crescente') antes de clicar DOWNLOAD; re-tenta o ícone
- expect_page com timeout 300s
- fallback: varre abas órfãs com URL amazonaws para o CNJ

Uso: fase_c2.py --staging DIR --cnjs lista.txt
"""
...
def achar_s3_orfa(ctx, numero):
    for p in ctx.pages:
        if "amazonaws.com" in p.url and numero in p.url:
            return p
    return None
...
def baixar(ctx, page, numero, staging):
    ...
    if "errorUnexpected" in pg.url or len(body) < 50:
        pg.close()
        return None, "viewer_nao_carregou"

    # abrir diálogo (até 3 tentativas, confirmando que abriu)
    dialog_ok = False
    for _ in range(3):
        pg.evaluate("""() => {
            var els = document.querySelectorAll('a, button, i, span');
            for (var i=0;i<els.length;i++){
                var t=((els[i].getAttribute('aria-label')||'')+' '+(els[i].title||'')).toLowerCase();
                if (t.indexOf('cone de download')>=0){ els[i].click(); return; }
            }
            // fallback: qualquer elemento curto com 'download'
            for (var i=0;i<els.length;i++){
                var t2=((els[i].getAttribute('aria-label')||'')+' '+(els[i].title||'')+' '+(els[i].innerText||'')).toLowerCase();
                if (t2.indexOf('download')>=0 && t2.trim().length<30){ els[i].click(); return; }
            }
        }""")
        time.sleep(4)
        if pg.evaluate("() => document.body.innerText.indexOf('Crescente') >= 0"):
            dialog_ok = True
            break
    if not dialog_ok:
        pg.close()
        return None, "dialogo_nao_abriu"
    ...
    url = None
    try:
        with ctx.expect_page(timeout=300000) as s3pop:
            clicked = pg.evaluate("""() => {
            ...
            if not clicked:
                raise RuntimeError("botao_download_nao_encontrado")
        s3 = s3pop.value
        for _ in range(90):
        ...
        orfa = achar_s3_orfa(ctx, numero)
        if orfa:
            url = orfa.url
            orfa.close()
        else:
            pg.close()
            return None, f"sem_aba_s3: {str(e)[:60]}"
    pg.close()
    if not url or "amazonaws.com" not in url:
        return None, f"url_inesperada: {(url or '')[:80]}"
    ...
    with sync_playwright() as pw:
        ...
        for numero in cnjs:
            ...
            if rec.get("pdf") and Path(rec["pdf"]).exists():
                log(f"[C2] {numero} já tem pdf — skip")
                continue
            log(f"[C2] {numero}")
            try:
