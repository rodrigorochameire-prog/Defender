...
export async function getOccupancyReport(
  ...
  const checkIns = await dbInstance
    .select({
      date: sql<string>`DATE(${daycareUsage.checkInTime})`,
      count: sql<number>`COUNT(*)`,
      revenue: sql<number>`SUM(COALESCE(${daycareUsage.amountCents}, 0))`,
    })
    .from(daycareUsage)
    .where(
      and(
        between(daycareUsage.checkInTime, startDate, endDate),
        isNotNull(daycareUsage.checkInTime)
      )
    )
    .groupBy(sql`DATE(${daycareUsage.checkInTime})`)
    .orderBy(sql`DATE(${daycareUsage.checkInTime})`);
